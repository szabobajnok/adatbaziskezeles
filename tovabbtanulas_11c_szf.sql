...
CREATE TABLE `altalanos_iskola` (
  ...
  `altalanos _iskola_címe` varchar(255) CHARACTER SET utf8 COLLATE utf8_hungarian_ci NOT NULL
...
INSERT INTO `altalanos_iskola` (`id_altalanos_iskola`, `altalanos_iskola_neve`, `altalanos _iskola_címe`) VALUES
(1, 'Csillagvár Általános Iskola', 'Kocka u. 14.'),
(2, 'Napsugár Általános Iskola', 'Gyöngyvirág u. 4.'),
(3, 'Tóparti Általános Iskola', 'Strand u. 23.');
...
CREATE TABLE `tanulo` (
  ...
  `tanulo_neve` varchar(255) COLLATE utf8mb4_hungarian_ci NOT NULL,
...
--
ALTER TABLE `jelentkezes`
  ADD CONSTRAINT `jelentkezes_ibfk_1` FOREIGN KEY (`tanulo_id`) REFERENCES `tanulo` (`id_tanulo`),
  ADD CONSTRAINT `jelentkezes_ibfk_2` FOREIGN KEY (`kozepiskola_id`) REFERENCES `kozepiskola` (`id_kozepiskola`);
